        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Create, prioritize, and track daily tasks</p>
                </div>
                ...
                <Card className="bg-white/60 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800">
                    <CardContent className="pt-6">
                        <div className="flex justify-between text-sm mb-2">
                            <span className="text-slate-500 dark:text-slate-400">{stats.completed} of {stats.total} tasks completed</span>
                            <span className="text-foreground font-medium">{Math.round((stats.completed / stats.total) * 100)}%</span>
                        </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <Input placeholder="Search tasks..." value={taskSearch} onChange={(e) => setTaskSearch(e.target.value)} className="pl-10 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-foreground" />
                </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="border-slate-200 dark:border-slate-800 hover:bg-transparent">
                                <TableHead className="text-slate-500 dark:text-slate-400 w-12">Done</TableHead>
                        ...
                        <TableBody>
                            {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                                ...
                                <TableRow className="border-slate-200 dark:border-slate-800">
                                    <TableCell colSpan={5} className="text-center py-12 text-slate-400">No tasks found</TableCell>
                                </TableRow>
                            ) : tasks?.map((task) => (
                                <TableRow key={task.id} className={`border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 ${task.completed ? 'opacity-60' : ''}`}>
                                    <TableCell>
                                        <button onClick={() => toggleMut.mutate({ id: task.id, completed: !task.completed })} className="cursor-pointer">
                                            {task.completed ? <CheckCircle className="h-5 w-5 text-emerald-500" /> : <Circle className="h-5 w-5 text-slate-400" />}
                                        </button>
                                    </TableCell>
                                    <TableCell className="text-slate-600 dark:text-slate-300">{task.date}</TableCell>
                                    <TableCell className={`text-foreground ${task.completed ? 'line-through' : ''}`}>{task.text}</TableCell>
                                    <TableCell>
            ...
            <Dialog open={formOpen} onOpenChange={setFormOpen}>
                <DialogContent className="sm:max-w-[500px] bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-foreground">
                    <DialogHeader><DialogTitle>New Task</DialogTitle></DialogHeader>
                    <form onSubmit={form.handleSubmit((v) => createMut.mutate(v))} className="space-y-4 mt-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label>Date</Label>
                                <Input type="date" className="bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700" {...form.register('date')} />
                            </div>
                            <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea placeholder="What needs to be done..." className="bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 min-h-[80px]" {...form.register('text')} />
