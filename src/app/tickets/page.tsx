        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-foreground tracking-tight">IT Ticketing</h1>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Manage and assign IT support tickets</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                {statCards.map((c) => (
                    <div key={c.label} className={cn('relative overflow-hidden rounded-2xl border bg-white dark:bg-slate-900 shadow-sm p-4',
                        c.label === 'Overdue' && overdueCount > 0 ? 'border-red-200 dark:border-red-900' : 'border-slate-200 dark:border-slate-800')}>
                        <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                                ...
                                {statsLoading ? <Skeleton className="h-7 w-12" /> : (
                                    <p className={cn('text-2xl font-bold tabular-nums', c.label === 'Overdue' && overdueCount > 0 ? 'text-red-500' : 'text-foreground')}>{c.value}</p>
                                )}
                            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-5">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">SLA Compliance</p>
                            </div>
                            <p className={cn('text-4xl font-bold tabular-nums', slaCompliance >= 80 ? 'text-emerald-500' : slaCompliance >= 60 ? 'text-amber-500' : 'text-red-500')}>{slaCompliance}%</p>
                            <p className="text-xs text-slate-400 mt-1.5">Tickets resolved within SLA</p>
                        </div>
                        <div className={cn('rounded-2xl border bg-white dark:bg-slate-900 shadow-sm p-5', overdueCount > 0 ? 'border-red-200 dark:border-red-900' : 'border-slate-200 dark:border-slate-800')}>
                            ...
                            <p className="text-xs text-slate-400 mt-1.5">{overdueCount === 0 ? 'All tickets within SLA' : 'Require immediate attention'}</p>
                        </div>
                        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-5">
                            <div className="flex items-center gap-2 mb-3">
                                <Timer className="h-4 w-4 text-blue-500" />
                                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Avg Resolution</p>
                            </div>
                            <p className="text-4xl font-bold text-foreground tabular-nums">{avgResolutionHours !== null ? `${avgResolutionHours}h` : '–'}</p>
                            <p className="text-xs text-slate-400 mt-1.5">Based on resolved tickets</p>
                        </div>
                    </div>

                    {/* Resolution Rate */}
                    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-5">
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Resolution Rate</p>
                        <div className="flex items-end gap-3 mb-4">
                            <span className="text-5xl font-bold text-foreground tabular-nums">{resolutionRate}%</span>
                            <span className="text-slate-400 text-sm pb-1">{(stats?.resolved ?? 0) + (stats?.closed ?? 0)} of {stats?.total ?? 0} resolved</span>
                        </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-5">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Ticket Volume — Last 7 Days</p>
                        </div>
                        <ResponsiveContainer width="100%" height={200}>
                            <LineChart data={weeklyData}>
                                ...
                                <Tooltip contentStyle={{ borderRadius: '10px', fontSize: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }} labelStyle={{ fontWeight: 700 }} />
                                <Line type="monotone" dataKey="count" name="Tickets" stroke="#10b981" strokeWidth={2.5} dot={{ r: 4, fill: '#10b981', strokeWidth: 0 }} activeDot={{ r: 6, strokeWidth: 0 }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    {/* Category Breakdown */}
                    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-5">
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Tickets by Category</p>
                        {categoryBreakdown.length === 0 ? (
                            <p className="text-slate-400 text-center py-8 text-sm">No tickets yet</p>
                        ) : (
                                    ...
                                    return (
                                        <div key={key} className="space-y-1.5">
                                            <div className="flex justify-between text-sm">
                                                <span className="font-semibold text-foreground">{cfg?.icon} {cfg?.label || key}</span>
                                                <span className="text-slate-400 tabular-nums">{count} <span className="text-slate-300 dark:text-slate-600">({pct}%)</span></span>
                                            </div>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                                        ...
                                        return (
                                            <TableRow key={ticket.id} className={cn('group border-l-2',
                                            ticket.priority === 'critical' ? 'border-l-red-500' :
                                            ticket.priority === 'high' ? 'border-l-orange-400' :
                                            ticket.priority === 'medium' ? 'border-l-sky-400' : 'border-l-slate-200 dark:border-l-slate-700',
            ...
            <Dialog open={formOpen} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-[700px] p-0 overflow-hidden gap-0 max-h-[92vh] flex flex-col">
                    {/* Gradient header */}
                    <div className={cn('px-6 py-5 shrink-0', editingTicket
                        ? editingTicket.priority === 'critical' ? 'bg-gradient-to-r from-red-600 to-red-700'
                        : editingTicket.priority === 'high' ? 'bg-gradient-to-r from-orange-500 to-orange-600'
                        : 'bg-gradient-to-r from-slate-700 to-slate-900'
                        : 'bg-gradient-to-r from-emerald-600 to-teal-700'
                    )}>
                        <DialogTitle className="text-white text-lg font-bold">
                            {editingTicket ? `Ticket #${editingTicket.number}` : 'New Ticket'}
                        </DialogTitle>
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                            {editingTicket?.sentiment && editingTicket.sentiment !== 'neutral' && editingTicket.sentiment !== 'positive' && (
                                <span className="text-xs font-semibold text-white/80">
                                    {sentimentConfig[editingTicket.sentiment]?.icon} {sentimentConfig[editingTicket.sentiment]?.label} sentiment
                                </span>
                            )}
                            {editingTicket?.due_date && (() => {
                                const s = getSlaStatus(editingTicket);
                                return s !== 'done' ? (
                                    <span className={cn('text-xs font-bold px-2 py-0.5 rounded-full', s === 'overdue' ? 'bg-white/20 text-white' : 'text-white/70')}>
                                        {s === 'overdue' ? '🚨 SLA OVERDUE' : `SLA: ${new Date(editingTicket.due_date).toLocaleString()}`}
                                    </span>
                                ) : null;
                            })()}
                            {!editingTicket && <p className="text-emerald-100 text-sm">Fill in the details to create a new support ticket.</p>}
                        </div>
                    </div>

                    {/* Tab bar — only when editing */}
                    {editingTicket && (
                        <div className="flex items-center gap-0.5 px-6 pt-3 pb-0 border-b border-slate-200 dark:border-slate-800 shrink-0">
                            {(['details', 'comments', 'activity'] as const).map(tab => (
                                <button
                                    key={tab}
                                    onClick={() => setDialogTab(tab)}
                                    className={cn('flex items-center gap-1.5 px-4 py-2 text-sm font-semibold border-b-2 transition-all -mb-px',
                                        dialogTab === tab
                    ...
                    {editingTicket && dialogTab === 'comments' && (
                        <div className="flex flex-col flex-1 min-h-[400px] overflow-hidden">
                            <ChatInterface
                                id={editingTicket.id}
                                isAdmin={true}
                                profile={profile}
                ...
                <DialogContent className="sm:max-w-[480px] p-0 overflow-hidden gap-0">
                    <div className="bg-gradient-to-r from-purple-600 to-purple-800 px-6 py-5">
                        <DialogTitle className="text-white font-bold flex items-center gap-2">
                            <GitMerge className="h-5 w-5" /> Merge Ticket #{mergingTicket?.number}
                        </DialogTitle>
                        <p className="text-purple-100 text-sm mt-0.5">Combine this ticket into another open ticket.</p>
                    </div>
            ...
            <Dialog open={cannedResponsesOpen} onOpenChange={setCannedResponsesOpen}>
                <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden gap-0">
                    <div className="bg-gradient-to-r from-slate-700 to-slate-900 px-6 py-5">
                        <DialogTitle className="text-white font-bold">IT Macros</DialogTitle>
                        <p className="text-slate-300 text-sm mt-0.5">Canned responses for common resolutions.</p>
                    </div>
