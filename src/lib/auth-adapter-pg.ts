    ...
    const adapter: Adapter = {
        async createUser(user: Omit<AdapterUser, "id">) {
            ...
            await execute(
                ...
                id, user.name, user.email, user.emailVerified?.toISOString() ?? null, user.image ?? null, (user as any).role || 'USER'
            );
        ...
        async updateUser(user: Partial<AdapterUser> & { id: string }) {
            ...
            if (user.emailVerified !== undefined) { fields.push('"emailVerified" = ?'); params.push(user.emailVerified?.toISOString() ?? null); }
        ...
        async linkAccount(account: AdapterAccount) {
            await execute(
                `INSERT INTO "Account" (id, "userId", type, provider, "providerAccountId", refresh_token,
