...
function serializeMachine(m: any): MachineRequest {
    return {
        ...
        notes: m.notes ?? '',
        resolution_notes: m.resolution_notes ?? null,
...
export async function addMachineRequest(input: CreateMachineInput): Promise<MachineRequest> {
    ...
    await execute(
        `INSERT INTO machine_requests (
            id, date, requester_name, user_name, work_email, reason, importance, status, notes, resolution_notes, internal_notes, created_by, created_at, updated_at, item_type, supply_name, item_count, requested_from
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        id,
        input.date ?? null,
...
export async function updateMachine(id: string, data: Partial<CreateMachineInput & { status: MachineStatus }>): Promise<void> {
    ...
    if (data.resolution_notes !== undefined) { fields.push('resolution_notes = ?'); params.push(data.resolution_notes); }
